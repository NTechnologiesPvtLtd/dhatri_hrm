...
public class UpdateServlet extends HttpServlet {
	...
	if(currentCTC!=null && !currentCTC.isEmpty()){
		 ...
		 * 
		 * emp.setNoticePeriod(Integer.parseInt(noticePeriod)); } catch
	...
	emp.setNoticePeriod(Integer.parseInt(noticePeriod));
		 ...
		 * 
		 * emp.setExpectedCTC(Double.parseDouble(expectedCTC)); } catch
		...
		emp.setPermanentAddress(permanentAddress);
		emp.setLocalAddress(localAddress);
		emp.setMobileNumber(HRMUtil.parseLong(mobileNumber));
		emp.setAlternateMobileNumber(HRMUtil.parseLong(alternateMobileNumber));
		emp.setAadharNo(HRMUtil.parseLong(aadharNo));
		emp.setPanNumber(panNumber);
		emp.setPassportNumber(passportNumber);
		emp.setSchoolName(schoolName);
		emp.setTenthPassedOut(HRMUtil.parseInt(tenthPassedOut));
		emp.setIntermediateCollegeName(intermediateCollegeName);
		emp.setInterPassedOut(HRMUtil.parseInt(interPassedOut));
		emp.setGraduationPassedOut(HRMUtil.parseInt(graduationPassedOut));
		emp.setGraduationDetails(graduationDetails);
		emp.setBranch(branch);
		emp.setUniversity(university);
		emp.setCompanyName(companyName);
